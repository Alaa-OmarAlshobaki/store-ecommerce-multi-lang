...
class verificationCodeServ
{
    ...
    public function checkOTPCode ($code){
        ...
        if (Auth::guard()->check()) {
            $verificationData =         VerificationCode::where('verify_code',$verify_code)->delete();
     ...
     */
    public function removeOTPCode($verify_code){
        VerificationCode::where('verify_code',$verify_code)->delete();
