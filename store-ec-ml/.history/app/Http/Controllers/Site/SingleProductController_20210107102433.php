...
class SingleProductController extends Controller
{
    public function Index(Request $request, $slug)
    {
         ...
         */
        return $data['product_attribute']=Attribute::whereHas('Option',function($q) use($productId){
            $q->where('product_id',$productId)->whereHas('Product');})->get();
         
         
         
            $data['product_attribute']=Attribute::whereHas('Option',function($optionPdoduct) use($productId){
           $optionPdoduct->where('product_id',$productId);
        })->get();
