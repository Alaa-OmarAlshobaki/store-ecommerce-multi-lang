...
class ProductController extends Controller
{
    ...
    public function postCreateStepTwo(Request $request)
    {
        ...
        $request->session()->put('product', ['price' => $validatedData, 'product' => $products,]);
        $product = $request->session()->get('product');
    ...
    public function postCreateStepThree(Request $request)
    {
        $validatedData = $request->validate([
            'sku' => 'required',
            'mange_stock' => 'required',
            'qty' => 'required',
            'in_stock' => 'required',
        ]);
        $products = $request->session()->get('product');
        $request->session()->put('product', ['stoke' => $validatedData, 'product' => $products,]);
        $product = $request->session()->get('product');
        return view('admin.products.create-step4');
    }
}
