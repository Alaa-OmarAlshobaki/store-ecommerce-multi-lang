...
class ProductController extends Controller
{
    ...
    public function createStepOne()
    {
        try {
            ...
            return view('admin.products.create-step1',compact('data'))->with('generlaInf',$data);
    ...
    public function postCreateStepOne(ProductValidRequest $request)
    {
        try {
              ...
              if(empty($request->session()->get('generlaInf'))){
          return "uuuuuuuuuuuuuuuu";
        }else{
            $generlaInf = $request->session()->get('generlaInf');
            $generlaInf->fill($request);
            $generlaInf->session()->put('data', $data);
        }
