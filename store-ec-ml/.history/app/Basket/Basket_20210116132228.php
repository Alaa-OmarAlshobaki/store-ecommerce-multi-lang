	...
	*/
	...
	public function subTotalQuan()
	{
		...
		foreach ($this->all() as $item) {
			...
			$total += $item->getTotal(true)* $this->get($product)['quantity']; ;
		}
	...
	{
		//    foreach( $this->get($product) as $item){
		   
		// 	   $pric=Product::where('id',$item)->get();
		// 	   $item= $pric[0]->price;
		// 	   if ($this->has($product)) 
		// 	   $quantity = $this->get($product)['quantity'] + $quantity;
		...
		//    }
				   ...
				   $quantity = $this->storage->get($product)['quantity'] + $quantity;
